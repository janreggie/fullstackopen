...
const Weather = ({ city }) => {
  ...
  useEffect(() => {
    ...
    axios.get('https://api.openweathermap.org/data/2.5/weather?' + params.toString())
      .then(
        ...
        reason => console.log(`Couldn't get weather data from ${city}`, reason)
      )
  }, [city, API_KEY])

  console.log('weatherData before err check', weatherData)
  ...
  console.log('weatherData after err check', weatherData)
